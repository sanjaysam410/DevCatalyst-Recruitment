...
interface TrackPasswordGateProps {
    team: string;
    children: React.ReactNode;
}

export default function TrackPasswordGate({ team, children }: TrackPasswordGateProps) {
    ...
    // Use a unique session key for each track
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const res = await fetch("/api/auth/evaluation", {
                method: "POST",
            ...
            >
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 shadow-[0_0_40px_rgba(103,58,183,0.2)] p-8 sm:p-10">
                    ...
                    <p className="text-gray-400 text-sm text-center mb-8">
                        Enter the track-specific password to continue
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {error && (
                            <motion.p className="text-red-400 text-sm text-center" initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }}>
                                {error}
                            </motion.p>
                        )}
                        ...
                        <button
                            ...
                            className="w-full py-3 rounded-lg bg-[#673ab7] hover:bg-[#5e35b1] text-white font-semibold text-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
